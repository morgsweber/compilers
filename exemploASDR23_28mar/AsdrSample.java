  ...
  private void Cmd() {
   ...
   private void RestoIF() {
      ...
      // else if (laToken == FI){
         ...
         // alternativa 1. aceita prodção vazia como regra default
         if (debug) System.out.println("RestoIF -->    /* vazio */  ");

         // alternativa 2. testa se o la pertence ao Follow(RestoIF)
      }
       //  verifica(FI); // laToken = this.yylex();
      //   }
  ...
  public static void main(String[] args) {
     ...
     try {
         if (args.length == 0)
          ...
          // 1. inicializar o LookAhead token
          laToken = parser.yylex();          

          // 2. Ativa o método correspondente ao símbolo inicial de G
          parser.Prog();
     
          // 3. testa se não "sobrou" código...
          //    (fronteira contém toda cadeia de entrada)
          if (laToken== Yylex.YYEOF)
             System.out.println("\n\nSucesso!");
          else     
             System.out.println("\n\nFalhou - esperado EOF.");               
